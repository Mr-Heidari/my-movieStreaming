...
const SuggestedMovieCard = ({ item }: prop) => {
  const getGenres = () => {
  ...
  return (
    <main className="cursor-pointer h-[200px] md:h-[300px] bg-neutral-800 flex flex-row gap-2 sm:gap-5 mx-2 md:mx-20 rounded-lg overflow-hidden border-4 border-black/70  relative max-[480px]:justify-between">
      <section>
        <img
      ...
      <section className="flex flex-col flex-wrap max-md:pt-2 md:justify-around w-full ">
        <div className=" flex flex-col max-sm:flex-row md:flex-row gap-5 py-5 text-white max-[480px]:hidden">
          ...
          <div className="flex flex-col gap-2 my-auto z-20 ">
            ...
            <p className="text-xs  opacity-50 ">{getGenres()?.join(" | ")}</p>
          </div>
        </div>
